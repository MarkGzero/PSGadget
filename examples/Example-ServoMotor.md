...
- [Troubleshooting](#troubleshooting)
  - [Device appears twice in Get-FtdiDevice (VCP driver conflict)](#device-appears-twice-in-get-ftdevice-vcp-driver-conflict)
...
Get-FtdiDevice -ShowVCP | Format-Table Index, Type, SerialNumber, Flags
...
Get-FtdiDevice -ShowVCP | Format-Table
...
### Device appears twice in Get-FtdiDevice (VCP driver conflict)

Symptom: `Get-FtdiDevice -ShowVCP` shows the same device at two different
...
- **Check for VCP driver conflict first.** This is the most common cause of
  ...
  factory. Run `Get-FtdiDevice -ShowVCP | Format-Table` — if the device
...
- Verify the FTDI device listed with `Get-FtdiDevice` has `GpioMethod : MPSSE`
