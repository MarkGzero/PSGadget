...
- The duplicate COM port entry in `List-PsGadgetFtdi` is confusing or interfering
...
`List-PsGadgetFtdi`: once with driver `ftd2xx.dll` (D2XX, for PSGadget) and once
...
# Remove duplicate COM port in List-PsGadgetFtdi
