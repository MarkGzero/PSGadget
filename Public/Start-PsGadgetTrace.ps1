...
function Start-PsGadgetTrace {
    <#
    .SYNOPSIS
    Enables protocol-level tracing and opens a live colorized viewer on the session log.

    .DESCRIPTION
    Start-PsGadgetTrace activates [PROTO] entries in the unified session log
    ...
    Typical usage:
        Start-PsGadgetTrace              # enable tracing + open viewer
    ...
    if (-not $logger) {
        Write-Warning 'Start-PsGadgetTrace: logger not available'
        return
    }
